...
function GalleryMedia({ media, showCaption, showModelLink = false, showSocials, dataLayerPush, clickAction, index, mediaCount, onLoad, backgroundColor }) {
  let isSvg = false;
  ...
  const customFigStyles = media?.link?.includes('insidemaps') ? styles.figCaptionStyles : null;
  ...
  const openPinterest = () => {
    window.open(
      'http://www.pinterest.com/pin/create/button/?url=' + encodeURIComponent(src) + '&description=' + encodeURIComponent(caption),
      'sharer',
      'toolbar=0,status=0,width=626,height=436'
    );
    if (dataLayerPush) {
    ...
    <div className={styles.mediaWrapper}>
      ...
      <figure className={`${styles.media} ${showMedia ? styles.show : ''}`}>
        ...
        {(type === 'video' || type === 'walkthrough') && (
          <>
            <iframe src={iframeSrc} onLoad={onImageLoad} allow='autoplay; encrypted-media' loading='lazy' ref={imgRef} />
            <div className={`${styles.iframeCover} mediaIframeCover__adjust`} onClick={useCallback(imageClicked, [index])} />
          </>
        )}
        ...
        {caption && showCaption && (
          <figcaption className={`${styles.mediaCapInline} ${customFigStyles}`} style={{ backgroundColor: backgroundColor }}>
            {caption}
          </figcaption>
        )}
        ...
        <div className={styles.bottomRightNav}>
          ...
          {showSocials && (
            <div className={styles.mediaShareNav}>
              <button className={`${styles.mediaFacebookShare} ${styles.mediaShareButton} js-facebook-share-analytics-trig`} onClick={openFacebook} />
              ...
              <button className={`${styles.mediaPinterestShare} ${styles.mediaShareButton} js-pinterest-share-analytics-trig`} onClick={openPinterest} />
            </div>
          )}
        </div>
      </figure>
