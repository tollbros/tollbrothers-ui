...
export async function handleChatInit(endPoint, apiSfOrgId, apiSfName) {
  try {
    const response = await fetch(`${endPoint}/iamessage/api/v2/authorization/unauthenticated/access-token`, {
      method: 'POST',
      headers: {
...
export const startConversation = async (payload, retries = 2, retryDelay = 1000) => {
  ...
  const performRequest = async (remainingRetries) => {
    try {
      const response = await fetch(`${payload.endPoint}/iamessage/api/v2/conversation`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    fetchEventSource(`${endPoint}/eventrouter/v1/sse`, {
      ...
      onmessage: (msg) => {
        ...
        if (typeof handleChatMessage === 'function') {
          handleChatMessage(msg, firstName, lastName, accessToken, conversationId)
        } else {
      ...
      onerror(err) {
        if (err === 'FatalError' || retryErrorCount > 2 || retryErrorOnOpenCount > 2) {
...
export const endChat = async ({ accessToken, conversationId, endPoint, apiSfName }, retries = 2) => {
  const performRequest = async (remainingRetries) => {
    try {
...
export const getConversationHistory = async ({ conversationId, accessToken, endPoint }) => {
  const response = await fetch(
...
export const postMessage = async (payload) => {
  const response = await fetch(`${payload.endPoint}/iamessage/api/v2/conversation/${payload.conversationId}/message`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${payload.accessToken}`
